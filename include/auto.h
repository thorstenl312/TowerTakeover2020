...
void Auton(){
  if(Red1){
    ...
    arm.stop(hold);
    arm.resetRotation();
    splineRight(-30, 10, 200, 30); 
    positiveYForward(46,55,30);
    rollerSpin(100);
    negativeYBackward(20.6,70,30);
    turnRightO9(128.5);
    roller.spin(forward,100,pct);
    accelerate(100,30);
    wait(230,msec);
...
void Skills(){
  ...
  negativeYBackward(114.9,50,60);
  turnRightO(96.5,30,30);
  rollerSpin(60);
  positiveXForward(30.8,55,30);
  stopDrive(hold);
  wait(150,msec);
  negativeXBackward(27.5,45,30);
  task z(rollOut45);
  wait(150,msec);
  ...
  wait(350,msec);
  stopDrive(hold);
  RollerL.rotateFor(-600,degrees,70, velocityUnits::pct,false);
  RollerR.rotateFor(-600,degrees,70, velocityUnits::pct,true);

  //Deploy Stack In Goal
  task k(armDown2);
  negativeXBackward(-33,60,30);
  rollerSpin(60);
  //task::stop(k);
  turnRightO(238,30,60);
  accelerate(80,30);
  wait(220,msec);
  stopDrive(coast);
  while(BackR.velocity(rpm) > 5 && BackL.velocity(rpm) > 5) wait(20,msec);
